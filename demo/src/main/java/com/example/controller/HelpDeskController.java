...
public class HelpDeskController {
	...
	public ModelAndView addAppointment(@Valid Schedule schedule, BindingResult bindingResult) {
		ModelAndView modelAndView = new ModelAndView();
		List<Schedule> schedule2 =  scheduleService.findAll();
		boolean overlap = false;
		for (int i = 0; i < schedule2.size(); i++) {
		if (schedule2.get(i).getStartTime() != null) 
			if
		(isOverlapping (schedule2.get(i).getStartTime(), schedule2.get(i).getEndTime(), schedule.getStartTime(), schedule.getEndTime())) 
			overlap = true;
		}
		if 	(!overlap) 	scheduleService.saveSchedule(schedule);
		modelAndView.setViewName("helpdesk/addAppointment");
		return modelAndView;
	}
